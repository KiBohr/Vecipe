...
const RecipeItem = ({item} : IItemProps) => {
    return ( 
        <Link className="w-40 flex flex-col items-center text-blue transition ease-in-out hover:shadow-2xl hover:opacity-90" to={`/details/${item.id}`}>
        <div className=" flex flex-col items-center text-center gap-1 bg-white/50 pb-2 rounded-lg">
            ...
            <h2 className=" self-center text-sm">{item.name}</h2>
